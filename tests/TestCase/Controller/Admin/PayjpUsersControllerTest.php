...
class PayjpUsersControllerTest extends TestCase
{
    ...
    public function testIndex_keywordSearch(): void
    {
        ...
        $this->get('/payjp/admin/payjp-users?q=abc');
    ...
    public function testIndex_keywordSearchWithNoMatch(): void
    {
        ...
        $this->get('/payjp/admin/payjp-users?q=zzz-not-found');
    ...
    {
        $this->loginAsAdmin();
        $this->get('/payjp/admin/payjp-users?query=x');
        $this->assertResponseOk();
    }
    ...
    {
        $this->loginAsAdmin();
        $this->get('/payjp/admin/payjp-users/add');
        $this->assertResponseOk();
    ...
    {
        $this->loginAsAdmin();
        $this->get('/payjp/admin/payjp-users/edit/1');
        $this->assertResponseOk();
        $this->assertNotNull($this->viewVariable('statuses'), 'ステータスの選択肢が渡っていない');
    }
}
